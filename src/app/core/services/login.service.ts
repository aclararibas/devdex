...
export class LoginService {
  ...
  login(login: string) {
    ...
    this.githubService.getUser(login).subscribe((data: user) => {
      ...
      this.userService.loading = false;
      this._snackBar.open(this.translator.get('snackbar.userNotFound'), 'X', {
        duration: 1500,
  ...
  setTitle(name: string) {
    name != 'null' ? (window.document.getElementsByTagName('title')[0].innerHTML = `DevDex | ${name}`)
      : window.document.getElementsByTagName('title')[0].innerHTML = `DevDex`;
  }
}
